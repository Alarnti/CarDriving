...
public class CarState : MonoBehaviour {
	...
	private TransformState startStatePositionRotation;
	private GameObject[] checkpoints;
	private int checkpointsPassed = -1;
	...
	void Start () {
		...
		startStatePositionRotation = new TransformState(transform.position, transform.rotation);
	...
	private TransformState ResetFromLastCheckpoint() {

		if(checkpointsPassed == -1) {
			return SampleHistoryState();
		}

		TransformState sampleHistory = SampleHistoryState();

		Vector3 position = checkpoints[checkpointsPassed].transform.position;
		position.y = sampleHistory.position.y;
		return new TransformState(position, sampleHistory.rotation);
	}
	...
	void OnTriggerEnter(Collider other) {
		...
		case "Checkpoint":
			...
			if(other.name == checkpoints[(checkpointsPassed + 1) % checkpoints.Length].name) {
				checkpointsPassed = (checkpointsPassed + 1) % checkpoints.Length;
			}
			Debug.Log("Car passed checkpoint");
			break;
		case "FinishLine":
			if(checkpointsPassed == checkpoints.Length && false) {
				Finished = true;
				ResetCheckpoints();
